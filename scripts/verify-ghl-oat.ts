/**
 * READ-ONLY GoHighLevel verification for Organized Auto Transport (OAT).
...
async function ghlRead<T>(endpoint: string, hint: string): Promise<T> {
  ...
  if (!token) {
    throw formatError({
      ...
      hint: "Set the OAT Private Integration token in .env.",
    });
  }
...
function recommendPipeline(pipelines: Pipeline[]) {
  const preferred = pipelines.find((pipeline) => /quote|transport|lead|sales|pipeline/i.test(pipeline.name));
...
async function main() {
  console.log("=== OAT GoHighLevel READ-ONLY Verification ===\n");

  const mockMode = isMockMode();
  console.log(`1. Mock mode: ${mockMode ? "ON (token missing)" : "OFF"}`);
  if (mockMode) {
    throw new Error("Mock mode is ON. Set GHL_PRIVATE_INTEGRATION_TOKEN to the OAT token before verifying.");
  }
  ...
  if (locationId) {
    ...
    const location = await ghlRead<Record<string, unknown>>(
      ...
      "Token may lack locations.readonly scope, or GHL_LOCATION_ID may be wrong for OAT.",
    );
    const nested = location.location as Record<string, unknown> | undefined;
    const locationName = String(nested?.name ?? location.name ?? location.companyName ?? "Unknown");
    ...
    console.log("\n3. GHL_LOCATION_ID is blank — attempting location discovery is not supported safely here.");
    throw new Error("Set GHL_LOCATION_ID for OAT in .env, then re-run verification.");
  }

  console.log(`\n4. OAT location ID: ${locationId}`);
  ...
  const opportunityFields = customFields.filter(
    (field) => !field.model || /opportunity/i.test(field.model),
  );
  const allFieldsForQuoteMatch = customFields;
  ...
  console.log("\n9. Quote-related custom fields matched:");
  ...
  for (const label of QUOTE_RESULT_FIELD_LABELS) {
    ...
    const match = allFieldsForQuoteMatch.find((field) => fieldMatches(field, aliases));
    if (match) {
